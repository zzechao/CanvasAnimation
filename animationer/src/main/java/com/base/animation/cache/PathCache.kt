...
class PathCache {
     ...
     */
    fun conventKey(intervalDeal: Long, animPath: AnimPathObject): String {
        ...
        startPoints.map {
            key += "_"
            key += it.value.map {
                "start_${it.alpha}_${it.point.x}_${it.point.y}_${it.rotation}_${it.scaleX}_${it
                    .scaleY}_${it.interpolator.javaClass.name}"
            }
        }
        endPointsMap.map {
            key += "_"
